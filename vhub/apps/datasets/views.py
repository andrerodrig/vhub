...
class DatasetsViewSet(viewsets.GenericViewSet):
    ...
    def list(self, request: Request) -> Response:
        ...
        serialized = self.serializer_class(dataset_list, many=True, context={"request": request})
    ...
    def create(self, request: Request) -> Response:
        ...
        if serialized.is_valid():
            ...
            # Calling the method from the lib to insert the data from the dataset 
            # in the table Data.
            LibData.save_csv_data(
                serialized.data.get("file"),
                Data.objects.create,
...
class DatasetsDetailViewSet(viewsets.GenericViewSet):
    ...
    def retrieve(self, request: Request, pk: int) -> Response:
        ...
        if dataset:
            ...
            serialized = self.serializer_class(dataset, context={"request": request})
            return Response(data=serialized.data, status=status.HTTP_200_OK)
        return Response(status=status.HTTP_404_NOT_FOUND)
    
    def update(self, request: Request, pk: int) -> Response:
        """Endpoint to update a dataset."""
        dataset = self.get_queryset().filter(owner_id=request.user.id).get(pk=pk)
